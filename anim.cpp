...
ShapeData cylData[numCylinders];
vec3 cylLocation[numCylinders] = {
	vec3(-5.0, 0.2, -3.0),
	vec3(-1.0, 0.2, -3.0),
	vec3(5.0, 0.2, -3.0),
	vec3(-5.0, 0.2, 3.0),
...
const int numObjects = numSquares + numCylinders;
...
// Don't change.
//////////////////////////////////////////////////////
void drawCylinder(ShapeData cyl)
{
	set_colour(cyl.color[0], cyl.color[1], cyl.color[2]);
	model_view = view;
	model_view *= Translate(cyl.location[0], cyl.location[1], cyl.location[2]);
	model_view *= Scale(0.8f, 0.1f, 0.8f);
	model_view *= rotateX(90);
    glUniformMatrix4fv( uModelView, 1, GL_TRUE, model_view );
    glBindVertexArray( cyl.vao );
    glDrawArrays( GL_TRIANGLES, 0, cyl.numVertices );
}
...
void myinit(void)
{
	...
	// Cylinders
	generateCylinder(program, &cylData[0], cylLocation[0], cylColors[0]);
	for(int i = 1; i < numCylinders; i++)
		duplicateCylinder(program, &cylData[i], cylLocation[i], cylColors[i]);
	// Squares
...
void myMouseCB(int button, int state, int x, int y)
{
    ...
    if( Button == GLUT_LEFT_BUTTON && state == GLUT_UP )
    {
		...
		{
			index = i + numSquares;
			pickingColor = getColorByIndex(index);
			glUniform4fv(uPickColor, 1, pickingColor);
			drawCylinder(cylData[i]);
		}
		...
		if(pickedID < numSquares)
			index = pickedID;
		else if(pickedID < numObjects)
			index = pickedID - numSquares;
		else
			index = 999;

		if(index != 999)
		{
			if(currentObject == NO_OBJECT && pickedID >= numSquares)
			{
				cylData[index].color = vec3(0.0, 0.0, 0.0);
				currentObject = index;
			}
			else if(currentObject != NO_OBJECT && pickedID < numSquares)
			{
				cylData[currentObject].color = cylColors[currentObject];
				cylData[currentObject].location[0] = squareData[index].location[0]; // match x
				cylData[currentObject].location[2] = squareData[index].location[2]; // match z
